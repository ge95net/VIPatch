...
face_landmark_detector = function.get_landmark_detector(cfg, device)
...
for mask_path, is_3d, save_name in [('final_patch.png', False, 'adv'),
    ...
    mask_t = function.load_mask(cfg, os.path.join('../data/masks', mask_path), device)
    uv_mask = mask_t[:, 3] if mask_t.shape[1] == 4 else None
    applied = function.apply_mask(location_extractor, fxz_projector, img_t, mask_t[:, :3], uv_mask, is_3d=is_3d)
