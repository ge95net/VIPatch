    ...
    def forward(self, img_batch):
        if isinstance(self.face_align, FaceAlignment):
            ...
            with torch.no_grad():
                ...
                '''preds[..., 0] = preds[..., 0] / self.img_size_height *416
                preds[..., 1] = preds[..., 1] / self.img_size_width *416'''
        return preds


class FaceXZooProjector(nn.Module):
    def __init__(self, device, img_size, patch_size,config):
        ...
        self.img_szie = 128
        self.device = device
        self.uv_mask_src = Image.open('prnet/new_uv.png').convert('L')
        
        self.transform = transforms.Compose([#transforms.Resize(config.img_size),
                                               transforms.ToTensor()])
        self.uv_mask_src = self.transform(self.uv_mask_src).to(device).unsqueeze(0)
        ...
        self.uv_face_src = Image.open('prnet/uv_face_mask.png').convert('L')
        
        self.uv_face_src = self.transform(self.uv_face_src).to(
            device).unsqueeze(0)
        self.triangles = torch.from_numpy(np.loadtxt('prnet/triangles.txt').astype(np.int64)).T.to(device)
        self.minangle = -1 / 180 * math.pi
        self.maxangle = 1 / 180 * math.pi
        self.min_trans_x = -0.005
        self.max_trans_x = 0.005
        self.min_trans_y = -0.005
        self.max_trans_y = 0.005
    ...
    def forward(self, img_batch, landmarks, adv_patch, first,uv_mask_src=None, do_aug=False, is_3d=False,get_mask=False):
        ...
        #save_img1 = adv_patch[0,:,:,:].squeeze().cpu().detach().permute(1,2,0).numpy()
        
        
        #cv2.imwrite('processing_images/adv_patch.jpg',save_img1[:,:,::-1]*255)
        if not is_3d:
            ...
            #uv_mask_src = uv_mask_src.repeat(adv_patch.shape[0], 1, 1, 1)
            uv_mask_src = uv_mask_src.unsqueeze(0).permute(1,0,2,3)
        ...
        new_img_batches = img_batch.unsqueeze(1).repeat(1,self.config.num_aug,1,1,1)

        
        
        '''if do_aug:
            for num in range(self.config.num_aug):
                ...
                adv_patch_uv = texture_patch * uv_mask_src
                ...
                save_mask = face_mask[0,:,:,:].cpu().detach().numpy().transpose(1,2,0)
                adv_patch_cpu = adv_patch[0,:,:,:].squeeze().cpu().detach().permute(1,2,0).numpy()
                adv_patch_cpu = cv2.resize(adv_patch_cpu,(128,128))
                save_mask_patch = save_mask * adv_patch_cpu
        ...
        new_img_batches = new_image
        new_image.data.clamp_(0, 1)
        if get_mask==True:
            mask_of_new_image = new_image[0,0,:,:].clone()

            mask_of_new_image[mask_of_new_image<0.99] = 0
            mask_of_new_image[mask_of_new_image>=0.99] = 1
            '''for height in range(mask_of_new_image.shape[0]):
                for width in range(mask_of_new_image.shape[1]):
                    if mask_of_new_image[height,width] <0.99 :
                        mask_of_new_image[height,width] = 0
                    else:
                        mask_of_new_image[height,width] = 1'''
            mask_of_new_image = mask_of_new_image.unsqueeze(0)
            
            
            mask_of_new_image = mask_of_new_image.cpu().detach().permute(1,2,0).numpy()
            mask_of_new_image = cv2.medianBlur(mask_of_new_image, 5)
 
            mask_of_new_image = torch.from_numpy(mask_of_new_image).unsqueeze(0)
            ...
            '''
            for height in range(1,mask_of_new_image.shape[0]-1):
                for width in range(1,mask_of_new_image.shape[1]-1):
                    count_pix_1_up = 0
                    count_pix_1_down = 0
                    count_pix_1_left = 0
                    count_pix_1_right = 0
                    ...
                    if mask_of_new_image[height,width] == 0:
                        for step in range(5):
                            if mask_of_new_image[height-step,width] >0.95:
                                count_pix_1_up +=1
                            if mask_of_new_image[height+step,width]  >0.95:
                                count_pix_1_down +=1
                            if mask_of_new_image[height,width-step]  >0.95:
                                count_pix_1_left +=1
                            if mask_of_new_image[height,width+step]  >0.95:
                                count_pix_1_right +=1
                    elif mask_of_new_image[height,width] == 1:
                        if mask_of_new_image[height-1,width] ==0:
                            count_pix_0_up +=1
                        if mask_of_new_image[height+1,width] ==0:
                            count_pix_0_down +=1
                        if mask_of_new_image[height,width-1] ==0:
                            count_pix_0_left +=1
                        if mask_of_new_image[height,width+1] ==0:
                            count_pix_0_right +=1
                    if count_pix_0 >=2 :
                    ...
                    if count_pix_1_up > 0:
                        count += 1
                    if count_pix_1_down > 0:
                        count += 1
                    if count_pix_1_left > 0:
                        count += 1
                    if count_pix_1_right > 0:
                        count += 1
                    if count >= 2:
                        mask_of_new_image[height,width] = 1
                    '''
            ...
            #mask_of_new_image = F.interpolate(mask_of_new_image.unsqueeze(0),(416,416))
            
            
            save_img1 = new_image[0,:,:,:].squeeze().cpu().detach().permute(1,2,0).numpy()
            cv2.imwrite('processing_images/new_image.png',save_img1[:,:,::-1]*255)

        
        else:
            mask_of_new_image = None
        return new_img_batches, mask_of_new_image
    ...
    def align_patch(self, adv_patch, landmarks):
        ...
        if landmarks.shape[1] == 68:
            ...
            left_top = torch.stack((landmarks[:, 33, 0]-max_side_dist, landmarks[:, -1, 1]-self.mask_half_height), dim=-1)
            right_top = torch.stack((landmarks[:, 33, 0]+max_side_dist, landmarks[:, -1, 1]-self.mask_half_height), dim=-1)
            left_bottom = torch.stack((landmarks[:, 33, 0]-max_side_dist,  landmarks[:, -1, 1]+self.mask_half_height), dim=-1)
            right_bottom = torch.stack((landmarks[:, 33, 0]+max_side_dist, landmarks[:, -1, 1]+self.mask_half_height), dim=-1)
...
class PRN:
    ...
    def preprocess(self, img_batch, image_info):
        ...
        tform = get_perspective_transform(src_pts, dst_pts)
    ...
    def process(self, img_batch, image_info):
        cropped_image, tform = self.preprocess(img_batch, image_info)   #image_info: face landmarks
    ...
    def get_vertices(self, pos):
        all_vertices = pos.view(pos.shape[0], 3, -1)    #(4,3,65536)
...
class TotalVariation(nn.Module):
    ...
    def forward(self, adv_patch,mask):
        tv_patch = adv_patch * mask
       
        self.uv_mask_src =mask.squeeze(0).squeeze(0)
        self.number_of_pixels = torch.count_nonzero(self.uv_mask_src)
        self.save_grads = torch.zeros_like(self.uv_mask_src)
...
class NPSCalculator(nn.Module):
    ...
    def __init__(self, printability_file, patch_side,device):
        ...
        self.device = device
        self.printability_file = printability_file
        self.device = device

    def forward(self, adv_patch,shape):
        # calculate euclidian distance between colors in patch and colors in printability_array 
        # square root of sum of squared difference
        self.printability_array = nn.Parameter(self.get_printability_array(self.printability_file, shape),requires_grad=False).to(self.device)
        adv_patch = adv_patch.unsqueeze(0).permute(1,0,2,3,4)
        color_dist = (adv_patch - self.printability_array+0.000001)
        
        color_dist = color_dist ** 2
        color_dist = torch.sum(color_dist, 2)+0.000001 #original 1
       
        color_dist = torch.sqrt(color_dist)
        # only work with the min distance
        color_dist_prod = torch.min(color_dist, 1)[0] #original 0#test: change prod for min (find distance to closest color)
        #print('color_dist_prod=',color_dist_prod)
        # calculate the nps by summing over all pixels
        nps_score = torch.sum(color_dist_prod,1)#original 0
        nps_score = torch.sum(nps_score,1) #original 0
