import cv2
save_image1 = cv2.imread('datasets/thermal/rgb/train/36_1_2_1_1183_25_3.png')
save_image = cv2.resize(save_image1,(128,128))
cv2.imwrite('best_image1.jpg',save_image)
import sys
import os
...
class Evaluator:
    def __init__(self, config, best_patch) -> None:
        ...
        self.embedders = function.load_embedder(self.config.test_embedder_names, device=device)
        emb_loaders, self.test_loaders = function.get_test_loaders(self.config, self.config.test_celeb_lab.keys())

        self.target_embedding_w_mask, self.target_embedding_wo_mask = {}, {}
        for dataset_name, loader in emb_loaders.items():
            self.target_embedding_w_mask[dataset_name] = function.get_person_embedding(self.config, loader, self.config.test_celeb_lab_mapper[dataset_name], self.location_extractor,
                                                                                    self.fxz_projector, self.embedders, device, include_others=True)
            self.target_embedding_wo_mask[dataset_name] = function.get_person_embedding(self.config, loader, self.config.test_celeb_lab_mapper[dataset_name], self.location_extractor,
                                                                                     self.fxz_projector, self.embedders, device, include_others=False)

        self.random_mask_t = function.load_mask(self.config, self.config.random_mask_path, device)
        self.blue_mask_t = function.load_mask(self.config, self.config.blue_mask_path, device)
        self.face1_mask_t = function.load_mask(self.config, self.config.face1_mask_path, device)
        self.face3_mask_t = function.load_mask(self.config, self.config.face3_mask_path, device)
    ...
    def apply_all_masks(self, img_batch, adv_patch):
        img_batch_applied_adv = function.apply_mask(self.location_extractor,
                                                 self.fxz_projector, img_batch, adv_patch)
        img_batch_applied_random = function.apply_mask(self.location_extractor,
                                                    self.fxz_projector, img_batch,
                                                    self.random_mask_t)
        img_batch_applied_blue = function.apply_mask(self.location_extractor,
                                                  self.fxz_projector, img_batch,
                                                  self.blue_mask_t[:, :3],
                                                  self.blue_mask_t[:, 3], is_3d=True)
        img_batch_applied_face1 = function.apply_mask(self.location_extractor,
                                                   self.fxz_projector, img_batch,
                                                   self.face1_mask_t[:, :3],
                                                   self.face1_mask_t[:, 3], is_3d=True)
        img_batch_applied_face3 = function.apply_mask(self.location_extractor,
                                                   self.fxz_projector, img_batch,
                                                   self.face3_mask_t[:, :3],
